...
// 	r.HandleFunc("/tasks/{id:[0-9]+}", completeTaskHandler).Methods("PUT")
...
// 	// Fetch tasks from MongoDB
// 	collection := client.Database("taskdb").Collection("tasks")
// 	cursor, err := collection.Find(ctx, bson.M{}) // Adjust the query if needed
// 	if err != nil {
// 		http.Error(w, err.Error(), http.StatusInternalServerError)
// 		log.Println("Error fetching tasks from MongoDB:", err)
// 		return
...
// 	// Update the task as completed in MongoDB
// 	collection := client.Database("taskdb").Collection("tasks")
// 	_, err = collection.UpdateOne(ctx, bson.M{"id": id}, bson.M{"$set": bson.M{"completed": true}})
// 	if err != nil {
// 		http.Error(w, err.Error(), http.StatusInternalServerError)
// 		return
// 	}
